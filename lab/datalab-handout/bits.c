...
int bitXor(int x, int y) {
    // return 0 if x and y are both 1 or both 0
    return ~(x & y) & ~(~x & ~y);
}
/*
 ...
 */
int isTmax(int x) {
    // ~0x7fffffff = 0x7fffffff + 1 = 0x80000000
    // ~0xffffffff = 0xffffffff + 1 = 0x00000000
    return !(~x ^ (x + 1)) & !!(x + 1);
}
/*
...
int isAsciiDigit(int x) {
    // return 1 if highest bit of (x - 0x30) and (0x39 - x) are both 0
    int int_min = 1 << 31;
    return !(int_min & (x + (~0x30 + 1))) & !(int_min & (0x39 + (~x + 1)));
}
/*
...
int isLessOrEqual(int x, int y) {
    // return 1 if highest bit of (y - x) is 0
    return !(1 << 31 & (y + (~x + 1)));
}
// 4
/*
...
int logicalNeg(int x) {
    // highest bit of x or -x is 1 if x is not 0
    // right shift 31 bits to get 0xffffffff or 0x00000000
    return ((x | (~x + 1)) >> 31) + 1;
}
